...
void    GLUI_Scrollbar::do_click( void )
{
  ...
  float incr = direction * growth * modifier_factor * user_speed ;

  int new_int_val = int_val;
  float new_float_val = float_val;

  new_int_val += (int)incr;
  if (data_type==GLUI_SCROLL_INT) set_int_val(new_int_val);
  
  new_float_val += incr;
  if (data_type==GLUI_SCROLL_FLOAT) set_float_val(new_float_val);
...
void    GLUI_Scrollbar::increase_growth( void )
{
  ...
  // This is kind of a hack to keep int scrollbars from auto-incrementing
  // too quickly in the small range case, and prevent them going too slow in 
  // the big range case.  A timeout would be a better way to deal with the former.
  if (data_type==GLUI_SCROLL_INT && range > 100 && growth < 1) growth = 1;
  return;
}
