...
#include "../include/glui_internal.h" //"glui_internal.h" //PCH
#include "../include/glui_algebra3.h" //"algebra3.h"
...
{
	return vec3(0,0,a.n[VX]*b.n[VY]-b.n[VX]*a.n[VY]);
}
...
{
	mat3 m_tmp; s >> m_tmp[VX] >> m_tmp[VY] >> m_tmp[VZ];

	if(s) m = m_tmp; return s;
}*/
...
{
	vec4 t; t = v[i]; v[i] = v[j]; v[j] = t;
}

void mat4::swap_cols(int i, int j)
{
	double t; int k; for(k=0;k<4;k++)
	{
...
{
	mat4 m_tmp; s >> m_tmp[VX] >> m_tmp[VY] >> m_tmp[VZ] >> m_tmp[VW];
...
{
	return mat3(vec3(1,0,v[VX]),vec3(0,1,v[VY]),vec3(0,0,1));
}
...
mat3 rotation2D(const vec2 &Center, double angleDeg)
{
	...
	return mat3(
	vec3(c,-s,Center[VX] * (1-c)+Center[VY]*s),
	vec3(s,c,Center[VY] * (1-c)-Center[VX]*s),
	vec3(0,0,1));
}
...
{
	return mat3(
	vec3(scaleVector[VX],0,0),
	vec3(0,scaleVector[VY],0),
	vec3(0,0,1));
}
...
{
	return mat4(
	vec4(scaleVector[VX],0,0,0),
	vec4(0,scaleVector[VY],0,0),
	vec4(0,0,scaleVector[VZ],0),
	vec4(0,0,0,1));
}
